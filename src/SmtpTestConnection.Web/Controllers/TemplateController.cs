...
namespace SmtpTestConnection.Web.Controllers
{
    public class TemplateController : SmtpTestConnectionControllerBase
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(model.Response) && model.Response == "getkey")
            {
            ...
            {
                HttpClient client = InitializeHttpClient();
                string url = $"https://cdacollections.projectdriveng.com.ng/api/Job/customtemplates?apiKey={model.Response}";
